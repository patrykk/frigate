...
if [[ "${TARGETARCH}" == "arm64" ]]; then
  ...
  pip3 install pybind11 && ln -s /usr/local/lib/python3.13/dist-packages/pybind11 $EXT_PATH/pybind11
  ln -s /usr/include/python3.13 $EXT_PATH/python3.13
  ln -s /usr/include/aarch64-linux-gnu/NvOnnxParser.h /workspace/TensorRT/parsers/onnx/

  # Build wheel
  cd /workspace/TensorRT/python
  EXT_PATH=$EXT_PATH PYTHON_MAJOR_VERSION=3 PYTHON_MINOR_VERSION=13 TARGET_ARCHITECTURE=aarch64 TENSORRT_MODULE=tensorrt /bin/bash ./build.sh
